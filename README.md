...
afk（宿主 ts 程序，零 pi SDK 依赖）
  ...
  → issue 回报 comment：成功（分支 + compare 链接）/ 失败（阶段 + 退出码 + stderr 摘要 + 产物路径 + 重跑提示）
  → label 状态机：agent:todo → agent:done（成功）/ agent:failed（失败）
  → 失败现场归档 .afk/failed/<branch>/ + 删本地分支（改回 agent:todo 干净重跑）
```
...
- **失败可恢复**：失败时 worktree 归档到 `.afk/failed/<branch>/`（保留代码现场），本地分支删除；issue 上留失败回报（阶段/退出码/stderr 摘要/产物路径/重跑提示），改回 `agent:todo` 即可干净重跑
...
| `AFK_BASE_BRANCH`                                        | `main`                                       | 基线分支（worktree 起点 + compare 链接 base） |
| `AFK_SESSIONS_DIR`                                       | `.afk/sessions`                              | 会话 JSONL 落盘目录（`--mode json` 事件流）   |
| `AFK_FAILED_DIR`                                         | `.afk/failed`                                | 失败现场归档目录                              |
...
- ✅ 无人值守循环：常驻容器三阶段（planner → implementer → reviewer → push 分支）+ 成功/失败 issue 回报 + 失败归档 + 本地分支清理；依赖编排层安装一次
- ✅ `afk run "<prompt>"` 即席命令：宿主本地单会话 pi，实时透传 + 会话 JSONL 落盘 + 双超时
- ⏸️ planner 续跑重试（`pi --session` 喂回校验错误）、SIGINT 优雅打断；关 issue / 自动 merge / 开 PR（D1 未决）；交互模式（工具路由）、skills —— 后续轮次
