...
describe('failureComment', () => {
  const base = {
    ...
    logPath: '.afk/logs/afk-2026-08-14.log',
    sessionPath: '.afk/sessions/afk-issue-52-site-links-nav-implementer.jsonl',
    archivePath: '.afk/failed/afk/issue-52-site-links-nav',
  ...
  it('包含日志/会话/归档三条产物路径', () => {
    const body = failureComment(base)
    expect(body).toContain('.afk/logs/afk-2026-08-14.log')
    expect(body).toContain('.afk/sessions/afk-issue-52-site-links-nav-implementer.jsonl')
    expect(body).toContain('.afk/failed/afk/issue-52-site-links-nav')
  })
