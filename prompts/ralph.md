...
请完成下面这个 GitHub issue。你工作在独立分支 `{{BRANCH}}` 上，**只负责实现和提交，不推送、不开 PR、不关 issue**。
...
1. **探索** —— 仔细阅读 issue。如果引用了父 PRD 或相关文档，先读完。
   - 阅读项目领域/架构文档再动手：`CONTEXT.md`、`docs/adr/`（如存在）、项目的编码规范文件
   - 阅读相关源码和测试后再编辑
...
- 不要 push、不要创建或编辑 PR、不要关闭/修改 issue 状态、不要改动 label。
